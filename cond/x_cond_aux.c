...
/* update cond starting from stage idx */
...
void UPDATE_COND_BABT(int idx, struct OCP_QP *ocp_qp, struct STRMAT *BAbt2, struct STRVEC *b2, struct COND_QP_OCP2DENSE_WORKSPACE *cond_ws)
	...
	// recompute entire cond
	if(idx==0)
		{
		COND_BABT(ocp_qp, BAbt2, b2, cond_ws);
		return;
		}

	// from now assume idx>0
...
void UPDATE_COND_B(int idx, struct OCP_QP *ocp_qp, struct STRVEC *b2, struct COND_QP_OCP2DENSE_WORKSPACE *cond_ws)
	{

	// recompute entire cond
	if(idx==0)
		{
		COND_B(ocp_qp, b2, cond_ws);
		return;
		}
