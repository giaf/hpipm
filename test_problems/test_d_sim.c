...
void d_linear_ode(int t, double *x, double *xdot, void *param)
	{
	struct d_linear_system *ls = param;
	int ii, jj;
	int nx = ls->nx;
	double *Ac = ls->Ac;
	for(ii=0; ii<nx; ii++)
...
void d_erk1(int steps, double h, int nx, double *x0, double *xe, void (*ode)(int t, double *x, double *xdot, void *param), void *param, void *memory)
	{
	int ii, step;
	double t;
	double *K = memory;
...
void d_erk(struct d_erk_args *args, int steps, double h, int nx, double *x0, double *xe, void (*ode)(int t, double *x, double *xdot, void *param), void *param, void *memory)
	{

	int ns = args->ns;
	double *A_rk = args->A_rk;
	double *B_rk = args->B_rk;
	double *C_rk = args->C_rk;
	...
	for(step=0; step<steps; step++)
		...
		for(ss=0; ss<ns; ss++)
			...
			ode(t+h*C_rk[ii], xt, K+ii*nx, param);
			}
		for(ss=0; ss<ns; ss++)
			{
			b = h*B_rk[ss];
...
************************************************/	
	...
	double *A; d_zeros(&A, nx, nx);
	for(ii=0; ii<nx*nx; ii++)
		A[ii] = Ts*Ac[ii];
	expm(nx, A);

	double *B; d_zeros(&B, nx, nu);
...
************************************************/	
	...
	double *K1; d_zeros(&K1, nx, 1);
	
	double *x1; d_zeros(&x1, nx, 1);
	double *ex1; d_zeros(&ex1, nx, 1);

	d_erk1(steps, h, nx, x0, x1, &d_linear_ode, &ls, K1);

	for(ii=0; ii<nx; ii++)
		ex1[ii] = x1[ii] - xref[ii];

	printf("\nx rk1\n");
	d_print_mat(1, nx, x1, 1);
	printf("\nerror rk1\n");
	d_print_e_mat(1, nx, ex1, 1);
...
************************************************/	
	...
	struct d_erk_args args;
	args.ns = ns;
	args.A_rk = A_rk;
	args.B_rk = B_rk;
	args.C_rk = C_rk;

	double *memory4 = malloc((nx+nx*ns)*sizeof(double));
	
	double *x4; d_zeros(&x4, nx, 1);
	double *ex4; d_zeros(&ex4, nx, 1);

	d_erk(&args, steps, h, nx, x0, x4, &d_linear_ode, &ls, memory4);

	for(ii=0; ii<nx; ii++)
		ex4[ii] = x4[ii] - xref[ii];

	printf("\nx rk4\n");
	d_print_mat(1, nx, x4, 1);
	printf("\nerror rk4\n");
	d_print_e_mat(1, nx, ex4, 1);
